...
export const fieldClass =
  "w-full rounded-xl border border-ink/15 bg-marfil px-4 py-3 text-base text-ink placeholder:text-ink-soft/60 outline-none transition-[border-color,box-shadow] duration-200 focus:border-gold focus:ring-4 focus:ring-gold/15 disabled:opacity-60";
...
export const surfaceClass =
  "rounded-[1.75rem] border border-ink/10 bg-marfil/80 shadow-[0_18px_40px_-32px_rgba(23,23,23,0.5)]";
...
export function chipClass(active: boolean) {
  return [
    ...
    active ? "border-ink bg-ink text-ivory" : "border-ink/15 bg-marfil/60 text-ink-soft hover:border-gold hover:text-ink",
  ].join(" ");
}
