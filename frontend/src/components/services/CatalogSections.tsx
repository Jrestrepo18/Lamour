...
const CATEGORY_EYEBROW: Record<Locale, Record<string, string>> = {
  es: {
  ...
  en: {
    "eroticos-tantricos": "Our signature ritual",
    "terapias-especiales": "Special therapies",
    sensoriales: "Senses heightened",
    "relajacion-muscular": "Physical wellbeing",
  },
};
...
export async function CategoryNav({
  ...
  return (
    <nav
      aria-label={t("Categorías", "Categories")}
      ...
      <ul className="flex gap-1 overflow-x-auto overflow-y-hidden px-1.5 py-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {extra && (
          <li className="shrink-0">
            <Link href={href(extra.href)} className={`${chip} gap-1 text-bronze hover:bg-silk hover:text-ink`}>
              {extra.label}
 ...
 */
export function catalogSections(categories: ServiceCategory[], lang: Locale) {
  let light = 0;
  ...
  const sections = categories.map((category) => {
    ...
    return (
      <CategorySection
        key={category.id}
        category={category}
        eyebrow={CATEGORY_EYEBROW[lang][category.slug] ?? translator(lang)("Catálogo", "Menu")}
        tinted={tinted}
      />
    );
  });
 ...
 */
export async function AdultSectionDoor() {
          ...
          >
            <div className="max-w-xl">
              ...
              <p className="mt-3 text-base leading-relaxed text-ink-soft">
                {t(
                  "Nuestro ritual insignia y las experiencias más íntimas de L'AMOUR tienen su propia sección, con precios, duración y todos los detalles.",
                  "Our signature ritual and L'AMOUR's most intimate experiences have their own section, with prices, durations and every detail.",
                )}
              </p>
            </div>
